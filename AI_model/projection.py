...
실측 근거 (동결 v1 모델 출력 60파일 × 4시나리오, 사영 후 4페어 최악 관통 cm):
  ...
  omega=1.8 K=8 : 0.000 / 0.000 / 0.000 / 0.000   <-- 측정된 최소 충분값
...
[현재 값이 24인 이유 — 정직한 서술] (2026-09-04, 8 -> 24)
   위 표는 K<=8까지만 잰 것이고 **24를 뒷받침하는 행은 없다.** K=8이 이미 전 시나리오
   0.000cm 이므로 24는 그 위에 얹은 여유폭이다. 표에 없는 값을 운용하고 있다는 사실을
   여기 남겨 둔다 — 되돌릴 근거가 필요하면 K=8 행이 그대로 남아 있다.

[K=24의 실측 비용 — 위 '수렴 이후는 공짜'의 단서]
   '공짜'는 프레임이 실제로 수렴할 때의 이야기다. 관통이 윈도우 전체에 지속되면 수렴이
   늦어 비용이 K를 따라간다 (2026-09-04, CPU, 30프레임 윈도우 1회, K=24):
       위반  0/30 : 중앙값  3.83 ms / 최악  4.83 ms
       위반 22/30 : 중앙값 35.04 ms / 최악 43.68 ms
       위반 30/30 : 중앙값 72.44 ms / 최악 87.35 ms
   30fps 예산 33.3 ms를 2.6배 초과한다. 팔짱·손 모으기처럼 관통이 수십 초 지속되는
   포즈에서는 이 구간이 계속된다. 실시간 스트리밍의 지연 상한을 잡을 때
   'K=8로 되돌리기'와 '여유폭 유지' 사이의 교환을 다시 판단해야 한다."""

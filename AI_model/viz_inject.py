...
실행 방법은 viz_motion.py 와 같은 두 가지를 모두 지원한다.

  (A) 소스 설정 + VS Code 실행 — 아래 '소스 설정' 블록을 고치고 Run(Ctrl+F5).
  (B) 명령줄 인자 (소스 설정보다 우선):
        python AI_model/viz_inject.py                    # 소스 설정대로
        python AI_model/viz_inject.py both               # 두 유형 gif 모두 생성
        python AI_model/viz_inject.py transient
        python AI_model/viz_inject.py persistent --seed 1234
        python AI_model/viz_inject.py both --out demo_results --show

  .vscode/launch.json 에 F5 구성도 등록되어 있다.

⚠️ 좌표 규약: 주입/물리(FK)는 Unity(Y-Up) 텐서에서 그대로 수행하고, 화면에 그릴 때만
   viz_motion 의 어댑터로 Z-Up 변환한다 (학습 경로와 동일한 규약).
"""
import argparse
import os
import random
...
def main(argv=None):
    ...
    if args.mode is None:
        print(f"ℹ️ 인자가 없어 소스 설정을 사용합니다 — INJECT_MODE='{mode}', "
              f"seed={args.seed} (바꾸려면 viz_inject.py 상단을 편집하세요).\n")

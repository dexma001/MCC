...
두 가지 뷰어를 제공한다.
  ...
  - COMPARE: Before(손상 입력) / After(모델 출력)를 나란히 재생하고,
             물리 엔진과 동일한 기준으로 침투 깊이를 색·수치로 표시한다.

실행 방법은 두 가지이며, 둘 다 지원한다.

  (A) 소스 설정 + VS Code 실행 — 아래 '소스 설정' 블록의 VIEW_MODE 등을 편집하고
      VS Code에서 그냥 Run(Ctrl+F5)하거나 `python AI_model/motion_viz.py` 실행.
      인자를 하나도 주지 않으면 소스 설정이 그대로 적용된다.

  (B) 명령줄 인자 — 소스를 고치지 않고 일회성으로 모드를 바꾼다. 인자를 주면
      소스 설정보다 인자가 우선한다.
        python AI_model/motion_viz.py single           # 무작위 파일 재생
        python AI_model/motion_viz.py single <a.pt>    # 특정 파일 재생
        python AI_model/motion_viz.py compare                        # demo_results/
        python AI_model/motion_viz.py compare --results inference_results
        python AI_model/motion_viz.py compare --save demo_results/compare.gif
...
SAVE_ANIMATION_PATH = "demo_results/compare.gif"    # 경로를 주면 gif로도 저장 (발표 슬라이드용).
...
def visualize_single(target_file="", pt_dir=PROCESSED_PT_DIR):
    ...
    def update(frame_idx):
        ...
        frame_pos_np = get_pos_tensor(motion_py[frame_idx], offsets_py).numpy()
...
def depth_style(depth, base_bone_color='dimgray', base_cap_color='dodgerblue'):
    ...
    COMPARE 뷰어와 inject_viz.py가 공유한다 — 색 규칙이 갈라지면 같은 깊이가
    화면마다 다르게 보이므로 한 곳에서만 정의한다.
    """
    if depth <= 0.0:
...
def visualize_compare(results_dir="demo_results", save_path=""):
    ...
    offsets_py = convert_offsets_to_python(_load_offsets_or_exit())
    physics_engine = DifferentiablePhysics(PARENTS, BONE_RADII).to('cpu')
...
def main(argv=None):
    ...
    if not args.mode:
        print(f"ℹ️ 인자가 없어 소스 설정을 사용합니다 — VIEW_MODE='{mode}' "
              f"(모드를 바꾸려면 motion_viz.py 상단의 VIEW_MODE를 편집하세요).")
